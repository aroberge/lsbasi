...
class LexerTestCase(unittest.TestCase):
    def makeLexer(self, text):
        from my_spi import Lexer
        lexer = Lexer(text)
        return lexer

    def test_lexer_integer(self):
        from my_spi import INTEGER
    ...
    def test_lexer_new_tokens(self):
        from my_spi import ASSIGN, DOT, ID, SEMI, BEGIN, END
        records = (
            (':=', ASSIGN, ':='),
            ('.', DOT, '.'),
            ('number', ID, 'number'),
...
class InterpreterTestCase(unittest.TestCase):
    def makeInterpreter(self, text):
        from my_spi import Lexer, Parser, Interpreter
        lexer = Lexer(text)
        parser = Parser(lexer)
